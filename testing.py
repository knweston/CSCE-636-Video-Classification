...
for i in tqdm(range(test_videos.shape[0])):
    ...
    timestamps = []
    # removing all other files from the testing_frames folder
    files = glob('testing_frames/*')
    for f in files:
        os.remove(f)
    while(cap.isOpened()):
        ...
        if (frameId % math.floor(frameRate) == 0):
            ...
            # storing the frames of this particular video in testing_frames folder
            filename ='testing_frames/' + videoFile.split('/')[1].split(' ')[0] + "_frame%d.jpg" % count;count+=1
            cv2.imwrite(filename, frame)
    cap.release()

    # reading all the frames from testing_frames folder
    images = glob("testing_frames/*.jpg")
    ...
    for i in tqdm(range(test.shape[0])):
        ...
        img = image.load_img('testing_frames/'+test['image'][i], target_size=(224,224,3))
        ...
        if prediction == 0 and actual[i] == "MoppingFloor":
            num_total_correct += 1
        if prediction == 1 and actual[i] == "NotMopping":
            num_total_correct += 1
